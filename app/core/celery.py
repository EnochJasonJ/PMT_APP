...
redis_pass = quote_plus(os.getenv("REDIS_PASSWORD", ""))  # Redis password for authentication
...
# Create a singleton engine for Celery to prevent connection leaks
_celery_engine = get_engine(
    SQLALCHEMY_DATABASE_URL,
    echo=DB_CEL_ECHO,
    pool_size=DB_CEL_POOL_SIZE,
    max_overflow=DB_CEL_MAX_OVERFLOW,
    pool_timeout=DB_CEL_POOL_TIMEOUT,
)
...
async def get_celery_db_session() -> AsyncIterator[AsyncSession]:
    """
    Async context manager for creating database sessions in Celery tasks using a shared engine.
    """
    session_maker = get_sessionmaker(_celery_engine)
